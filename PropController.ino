...
//float parcour[6] = {36.65024839882367, 2.696118238404322, 36.65008052996648, 2.695898256218098, 36.65004609914933, 2.6962362145702015};
float parcour[6] = {36.650212247652775, 2.696923787954538, 36.664283300753226, 3.1006547531403488, 36.664553317124174, 3.1007754525421585};
/*
36.65024839882367, 2.696118238404322      Ancre
36.64994174948469, 2.6960511831810936    parking
36.64986434736686, 2.6965541005283713   round-point
36.650212247652775, 2.696923787954538   Piscine
*/
...
void loop()
{
  ...
  */
  ...
  /* There are Two controllers, use only one of them*/
  
  
  /*The GO function is based on the ON/OFF controller with an offset of -10°(Bd) to 10° (Td)*/
  // Go(Bearing, Infos.yaw , Infos.RelativeBearing, Distance, Infos.flags);
  
  
  /* The Proportionel Controller with Kp=1*/
  float Cap_init 		= Heading;
  float Cap_Destination = Bearing;
  
  Cde_prop(Cap_Destination, 1, Cap_init, Infos.flags);
...
void Cde_prop(float Bearing, float Kp, float Offset, byte fg){
float Commande;
float Erreur;

Erreur   = Bearing - Offset;
Commande = Kp * Erreur;

// See in Flags That there is no Problem with Communication and other thing

if (fg & 0x1 || fg & 0x2 || fg & 0x4) {
 //There is a problem.
...
Commande = 90 + Commande;
GouverSVO.write(Commande);
 }
}



// The GO function Responsible for manouevring and Start/Stop Engine
